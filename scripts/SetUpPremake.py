#For the time being this is mostly a copy of hazel engine implementation, will do own implementation when I have time.
...
premakeExe = Path(f"./vendor/bin/premake/premake5.exe")
...
def InstallPremake():
    ...
    while not permissionGranted:
        ...
        DownloadFile(premakeZipUrls, premakePath)
        print("Extracting", premakePath)
        UnzipFile(premakePath, deleteZipFile=True)
        ...
        DownloadFile(premakeLicenseUrl, premakeLicensePath)
        print(f"Premake License file has been downloaded to '{premakeDirectory}'")

    return True

if (premakeExe.exists()):
    subprocess.call([os.path.abspath("./scripts/GenWin.bat"), "nopause"])
else:
    if (InstallPremake()):
        subprocess.call([os.path.abspath("./scripts/GenWin.bat"), "nopause"])
